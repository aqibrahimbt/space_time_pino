...
class BlackScholesEquation(DifferentialEquation):
    ...
    def __init__(self, r: float = 0.05, sigma: float = 0.2):
        ...
        if r < 0.0 or sigma < 0.0:
            raise ValueError("r and sigma must be non-negative")

        self._r = r
        self._sigma = sigma

        super(BlackScholesEquation, self).__init__(1, 1)

    @property
    def symbolic_equation_system(self) -> SymbolicEquationSystem:
        s = self._symbols.y[0]
        return SymbolicEquationSystem(
            [self._r * s - 0.5 * self._sigma**2 * s**2 -self._r]
        )
...
class MultiDimensionalBlackScholesEquation(DifferentialEquation):
    ...
    def __init__(self, r: float = 0.05, sigma: float = 0.2):
        ...
        super(MultiDimensionalBlackScholesEquation, self).__init__(2, 1)

    @property
    def symbolic_equation_system(self) -> SymbolicEquationSystem:
        s = self._symbols.y[0]
        return SymbolicEquationSystem(
            [self._r * s - 0.5 * self._sigma**2 * s**2]
        )
